...
sudo systemsetup -setcomputersleep Off > /dev/null 2>&1 ;ok
...
# does this work?
# running "Set timezone automatically based on location"
...
defaults write com.apple.dock largesize -float 80;ok
...
sudo mdutil -i on / > /dev/null 2>&1;ok
...
if [ "${CURRENT_PROFILE}" != "${TERM_PROFILE}" ]; then
  open "$_BOX_ROOT/lib/box-defaults/config/${TERM_PROFILE}.terminal";
...
open "$_BOX_ROOT/lib/box-defaults/config/Solarized Dark.itermcolors";ok
